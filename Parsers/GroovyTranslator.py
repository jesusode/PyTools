...
def translateCommentToGroovy(com):
...
def translateToGroovy(t):
    if t.data == 'instructions':
        ...
        while hasattr(t.children[cont],"data") and t.children[cont].data == "ambit":#ambit*
            translateToGroovy(t.children[cont])
            cont += 1
        ...
        if t.children[1] == ':':
            ...
            Config.prog_str += ' ' + t.children[1] + ' '
            translateToGroovy(t.children[2])
            Config.prog_str += t.children[3] + '\n'
        else: #todo
            translateToGroovy(t.children[1])
            if t.children[2] == 'for':
                Config.prog_str += ' ' +  t.children[2] + ' '
                translateToGroovy(t.children[3])
        ...
        while t.children[cont] == "catch":
            ...
            #Ajuste de indent
            Config.indent_str += "  "
            translateToGroovy(t.children[cont+1])
            #recuperar indent
            Config.indent_str = Config.indent_str[:-2]
            cont += 2
        if t.children[cont] != "end":#Hay finally
            ...
            #Ajuste de indent
            Config.indent_str += "  "
            translateToGroovy(t.children[cont+2])
            #recuperar indent
            Config.indent_str = Config.indent_str[:-2]          
        Config.prog_str += Config.indent_str + t.children[-1]
        ...
        while t.children[cont] == "elif":
            ...
            #Ajuste de indent
            Config.indent_str += "  "
            translateToGroovy(t.children[cont+3])
            #recuperar indent
            Config.indent_str = Config.indent_str[:-2]
            cont += 4
        Config.prog_str += Config.indent_str + '}' + t.children[cont] + ' {\n'
    ...
    # "generic" "[" boolexpr (("where"|"extends"|"from"|"requires") boolexpr)? (COMMA boolexpr (("where"|"extends"|"from"|"requires") boolexpr)?)* "]"
        ...
        translateToGroovy(t.children[2]) #boolexpr
        cont +=3
        while cont < len(t.children):
            if t.children[cont] == ']':
                Config.prog_str += t.children[cont]
        ...
        translateToGroovy(t.children[0]) #fun_header
        Config.prog_str += ' '
        translateToGroovy(t.children[1]) #fname
        cont = 2
        if t.children[2] == "()": #()
            Config.prog_str += t.children[2]
            cont +=1
        ...
        translateToGroovy(t.children[4])
        cont = 5
        while t.children[cont] == ",":
            Config.prog_str += t.children[cont] + ' '
            translateToGroovy(t.children[cont+1])
        ...
        Config.prog_str += translateCommentToGroovy(t.children[0])
        ...
        translateToGroovy(fixed[1]) #boolexpr_list
        Config.prog_str += ') {\n' #do
        #Ajuste de indent
        Config.indent_str += "  "
        translateToGroovy(fixed[3])#instructions, ignoramos end
        #recuperar indent
        Config.indent_str = Config.indent_str[:-2]
        Config.prog_str += Config.indent_str + "}"
    #El for deberia aceptar en la primera opcion i as generic
        ...
        #Ajuste de indent
        Config.indent_str += "  "
        translateToGroovy(t.children[-2])#instructions, ignoramos end
        #recuperar indent
        Config.indent_str = Config.indent_str[:-2]
        Config.prog_str += Config.indent_str + "}"
        ...
        cont = 0
        Config.prog_str += 'def ' + t.children[1]
        cont+=2
    ...
    # "{" pair (COMMA pair)* "}"  
        ...
        args = ""
        while t.children[cont] != "]":
            args += t.children[cont][1:] + ' '
            cont += 1
        Config.prog_str += args + "->\n"
        cont += 2
        translateToGroovy(t.children[cont])
        Config.prog_str += "}"
