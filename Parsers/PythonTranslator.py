...
def translateToPython(t):
    ...
    elif t.data == 'import_1': #"ambit* (import|package)" ID ("as" ID)? (COMMA ID ("as" ID)?)* 
        cont = 0
    ...
    elif t.data == 'import_2': #ambit* "from" ID "import" (ID|"*") ("as" ID)? 
        cont = 0
        while hasattr(t.children[cont],"data") and t.children[cont].data == "ambit":#ambit*
            translateToPython(t.children[cont])
            cont += 1
    ...
    elif t.data in ['_string','number','empty_list','empty_tuple']:
        if t.data == '_string':
    ...
    elif t.data == 'list_comprehension': #"[" boolexpr "for" ID(COMMA ID)* "in" boolexpr ("if" boolexpr)? "]" 
        Config.prog_str += t.children[0]
        translateToPython(t.children[1])
